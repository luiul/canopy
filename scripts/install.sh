...
# Builds canopy and installs it to ~/.local/bin (or $CANOPY_INSTALL_DIR),
# then code-signs it with a stable local certificate + identifier instead
# of leaving it plain ad-hoc-signed.
#
...
# granted to the old one. canopy needs it (for mycelium's AppleScript,
# backing jump's open-or-focus on every run: System Events for the VS
# Code window listing and raise, Ghostty's scripting bridge for
# terminal rows) on every run, so without a stable signature it'd need
...
DEST="${CANOPY_INSTALL_DIR:-$HOME/.local/bin}/canopy"

cd "$(dirname "$0")/.."

tmp=$(mktemp -t canopy-build)
go build -o "$tmp" ./cmd/canopy
